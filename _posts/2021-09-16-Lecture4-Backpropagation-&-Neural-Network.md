...
1.  예측한 레이블의 점수는 그대로이고 클래스별 점수가 증가하면, 손실 값의 크기가 감소한다. 아래 그래프는 이를 나타내었다.
...
$[1,1,1,1]$ 의 값을 가진 데이터 X 와 $W1, W2$ 을 각각 곱하면 손실값이 동일하게 1로 나오지만 $W1$ 처럼 가중치가 한 특징에만 집중되어 있는 경우 ***L2 정규화*** 를, $W2$ 처럼 가중치가 모든 특징에 골고루 분배되어 있으면 ***L1 정규화*** 를 사용하는 것이 좋다.
