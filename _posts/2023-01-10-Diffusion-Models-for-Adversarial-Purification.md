...
![](/assets/image/diffpure-2.png){: width="30%" height="30%"}{: .center}

  DM의 훈련은 다음과 같이 score function인 $s_\theta (\tilde{x}, t)$을 transition probability 인 $\bigtriangledown_\tilde{x} log_p_{0t}(\tilde{x}|x)$에 최대한 가까워 지도록 만드는 denosing score matching 방법을 이용한다.
  
![](/assets/image/diffpure-3.png){: width="30%" height="30%"}{: .center}
...
![](/assets/image/diffpure-4.png){: width="30%" height="30%"}{: .center}
  ...
  1. Forward process: 이 논문에서는 DM으로 VP-SDE를 사용한다. VP-SDE는 다음과 같은 공식으로 timestep $t*$에 노이즈가 추가된 이미지를 구할 수 있다.
  
![](/assets/image/diffpure-5.png){: width="30%" height="30%"}{: .center}

  2. Reverse process: Euler-Maruyama solver로 reverse-time SDE를 다음과 같이 풀 수 있다.
  
![](/assets/image/diffpure-6.png){: width="30%" height="30%"}{: .center}

  위 식에서 $sdeint$의 입력값은 차례대로 initial value, drift coefficient, diffusion coefficient, Wiener process, initial time, 와 end time 이다. $f_rev$와 $g_rev$는 다음과 같다.
  
![](/assets/image/diffpure-7.png){: width="30%" height="30%"}{: .center}

  여기서 핵심은 diffusion을 하는 timestep인 $t*$를 정하는 것이다. 최적의 $t*$은 다음과 같은 정리를 통해서 구할 수 있다.
...
![](/assets/image/diffpure-8.png){: width="30%" height="30%"}{: .center}

![](/assets/image/diffpure-9.png){: width="30%" height="30%"}{: .center}

  purified image가 최대한 clean image와 유사한 label semantics를 가져야 하므로 $\lVert \tilde{x}(0)-x \RVert$를 작게 하는 $t*$를 찾아야 한다. 그러기 위해서는 $\gamma(*t)$가 작아야 하고, 이는 곧 $t*$가 작아야 한다는 의미이므로 $t*$를 최대한 작게 설정해야 한다.
  
  첫번째와 두번째 정리를 통해 $t*$값에 따라 adversarial sample에서 perturbation을 purify하는 정도와 purified image가 가지는 label semantics간에는 trade-off가 있다는 것을 알 수 있다. 따라서 최적의 $t*$를 잘 선택하는 것이 중요하다. 이 논문에서는 perturbation이 작은 경우가 많다고 하였기 때문에 $t*$값을 작게 설정하였다.
...
![](/assets/image/diffpure-10.png){: width="30%" height="30%"}{: .center}
...
![](/assets/image/diffpure-11.png){: width="30%" height="30%"}{: .center}
...
![](/assets/image/diffpure-12.png){: width="30%" height="30%"}{: .center}

  마지막은 AutoAttack $l_\infty (\epsilon = 4/255) $을 ImageNet에 적용했을 때 결과이다. 마찬가지로 모든 classifier에서 SOTA임을 확인할 수 있다.

![](/assets/image/diffpure-13.png){: width="30%" height="30%"}{: .center}
