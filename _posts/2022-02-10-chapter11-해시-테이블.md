...
## Question 30: 중복 문자 없는 가장 긴 부분 문자열
...
class Solution:
    def topKFrequent(self, nums: List[int], k: int) -> List[int]:
        numbers_count = collections.Counter(nums)
        idx_list = []
        for idx in numbers_count:
            idx_list.append(idx)
        idx_list.sort(key = lambda x: numbers_count[x])
        idx_list = idx_list[::-1]
        return idx_list[0:k]
```
---
...
- sort() 에 람다 함수를 사용하여 숫자가 나타나는 횟수를 기준으로 숫자를 정렬하였다.
- 딕셔너리를 반복할때 키 값이 iterator가 된다.

#### 결과
Runtime: 100 ms, faster than 87.86% of Python3 online submissions for Top K Frequent Elements.
Memory Usage: 18.6 MB, less than 94.86% of Python3 online submissions for Top K Frequent Elements.
