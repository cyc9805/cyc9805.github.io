...
![](/assets/image/densepure-2.png){: width="30%" height="30%"}{: .center}
...
![](/assets/image/densepure-4.png){: width="30%" height="30%"}{: .center}
  ...
  여기서 이 논문에서는 다음 step에서 노이즈를 제거한 샘플 분포의 분산을 $/beta_i$ ($i$번째 step에 주입한 노이즈의 양) 으로 고정하고, 평균인 $/mu_/theta (x_i, i)$ 만을 구하는 것으로 간단히 하였다. 손실 함수를 계산하는 과정이 워낙 복잡하기 때문에 이 과정을 생략하고 최종적인 손실 함수만을 나타내면 다음과 같이 나타낼 수 있다.
  
![](/assets/image/densepure-5.png){: width="30%" height="30%"}{: .center}
...
![](/assets/image/densepure-7.png){: width="30%" height="30%"}{: .center}

### Theorem 3.1

  ***Perturbed된 데이터 포인트인 $x_{a,t}$가 reverse-SDE를 거치고 난 후 purify된 데이터 포인트 $/hat{x}$ 는 다음과 같은 조건부 확률분포를 가진다.***
  
![](/assets/image/densepure-8.png){: width="30%" height="30%"}{: .center}

위 공식을 통해 $\mathbb{P}$를 증가시키기 위해서는 $||x-x_a||$를 감소시켜야 하고, 이는 곧 $x$가 $x_a$ 주변으로 높은 밀도를 가져야 함을 알 수 있다. 따라서 다음과 같이 $\mathbb{P}$를 최대화하는 데이터 포인트 $x$를 선택하는 것으로 간단히 나타낼 수 있다.

![](/assets/image/densepure-9.png){: width="30%" height="30%"}{: .center}
...
![](/assets/image/densepure-10.png){: width="30%" height="30%"}{: .center}
...
![](/assets/image/densepure-11.png){: width="30%" height="30%"}{: .center}
...
![](/assets/image/densepure-12.png){: width="30%" height="30%"}{: .center}

***2. 또한 $x_a$는 다음과 같은 convex set에 포함될 때 $x_0$과 같은 label을 가진다.***

![](/assets/image/densepure-13.png){: width="30%" height="30%"}{: .center}

  여기서 1과 2의 다른 점은 2는 $x_0$과 같은 레이블을 가진 다른 데이터 포인트 $\tilde{x}$를 사용한다는 점이다. <br>

  결국 robust radius인 $r(G(x_0);t$를 찾는 것이 이 문제의 핵심이라고 할 수 있다. 여기서 한가지 주의해야 할 점이 있다. 바로 $D_sub(x_0 ; t)$는 convex여도 $D(G(x_0);t)$는 convex가 아니라는 점이다. 따라서 $D(G(x_0);t)$를 해결하기 위해서는 다음과 같은 방법으로 문제에 접근해야 한다. 

  1. non-convex optimization 방법을 이용해야 한다. 
  2. $D_sub(x_0 ; t)$는 convex이므로 convex optimization을 이용해 해결해서 $r(G(x_0);t$에 대한 lower bound를 찾을 수 있다.
...
![](/assets/image/densepure-14.png){: width="30%" height="30%"}{: .center}
...
### Theorem 3.4
  ***$\left \{\hat{x_\gamma} \right \}_{\gamma \in  [0, t]}$ 와 $\left \{x^{\theta}_\gamma \right \}_{\gamma \in  [0, t]}$ 가 각각 reverse-SDE, score-based diffusion model이라고 하면 이 두 분포의 KL-Divergence는 다음과 같이 나타낼 수 있다.***
  ![](/assets/image/densepure-15.png){: width="30%" height="30%"}{: .center}

  ![](/assets/image/densepure-16.png){: width="30%" height="30%"}{: .center}
...
설명하고 있으나 중요한 내용은 아니므로 생략하도록 하겠다. <br>
  다음은 DensePure의 pipeline에 대해 나타낸 figure이다.
    ...
    ![](/assets/image/densepure-17.png){: width="30%" height="30%"}{: .center}
...
![](/assets/image/densepure-18.png){: width="50%" height="50%"}{: .center}
...
![](/assets/image/densepure-19.png){: width="50%" height="50%"}{: .center}
...
![](/assets/image/densepure-20.png){: width="50%" height="50%"}{: .center}
