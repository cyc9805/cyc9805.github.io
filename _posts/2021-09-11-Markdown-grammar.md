...
\``` 이 문자가 코드 블록의 시작을 알리는 문자이나 맥북 키보드에서 보이지가 않는다. 혹시라도 이 문자를 맥북에서 치는 방법을 아는 분 있으면 연락 부탁드립니다!! 
...
\<cite>--- 태그와 \{{: .small}} 을 같이 써서 인용문 출저를 남길 수도 있다. 예시는 다음과 같다.

```python
<cite>공부하는 식빵맘</cite> --- 마크다운 문법 총 정리{{: .small}}
```
<cite>공부하는 식빵맘</cite> --- 마크다운 문법 총 정리{{: .small}}
